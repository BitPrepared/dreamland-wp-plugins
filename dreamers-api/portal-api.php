...
class Portal_API
{
    ...
    public function register_routes($routes)
    {
        ...
        $routes['/portal/profilo/(?P<id>\d+)'] = array(
            array(array($this, 'find_user'), WP_JSON_Server::READABLE)
        );
    ...
    public function find_user($id, $context = 'view') {

        _log('find user');

        $users = get_users(array('meta_key' => 'codicecensimento', 'meta_value' => $id));
        if ( count($users) == 1 ) {
            $utenteCercatoPrimario = $users[0]; //WP_User
            $uu = new \stdClass();
            $uu->user_id = $utenteCercatoPrimario->ID;
            $elencoMeta = get_user_meta($utenteCercatoPrimario->ID);
            $uu->meta = $elencoMeta;
            return $uu;
        } else {
            return "";
